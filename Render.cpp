...
void drawPlayer(RenderWindow& window, Player& p) {
   ...
   head.setPosition(sf::Vector2f(p.position.x, p.position.y));
    window.draw(head);

    RectangleShape body(Vector2f(4, 50));
    body.setFillColor(Color::White);
   body.setPosition(sf::Vector2f(p.position.x + 18, p.position.y + 40));
    window.draw(body);

    RectangleShape leg1(Vector2f(4, 35));
    leg1.setFillColor(Color::White);
    leg1.setPosition(sf::Vector2f(p.position.x + 18, p.position.y + 90));
   leg1.setRotation(sf::degrees(20));
    window.draw(leg1);

 RectangleShape leg2(Vector2f(4, 35));
    leg2.setFillColor(Color::White);
    leg2.setPosition(sf::Vector2f(p.position.x + 18, p.position.y + 90));
 ...
 void drawHealthBar(sf::RenderWindow& window, Player& p) {
    ...
    bgBar.setPosition(sf::Vector2f(p.position.x - 10, p.position.y - 20));
    window.draw(bgBar);

    sf::RectangleShape healthBar(sf::Vector2f(p.position.x - 10, p.position.y - 20));
    healthBar.setFillColor(sf::Color::Green);
    healthBar.setPosition(sf::Vector2f(p.health * 0.6f, 8));
